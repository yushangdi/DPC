...
// declear class "pts" which consists only of vector
class pts;
...
public:
	...
	void key_computation(const float cutoff) {
	...
	// memory release
	void result_release() { std::vector<int>().swap(result); }
};
...
// sort descending order of local density
bool desc_local_density_pts(const pts& l, const pts& r) { return l.local_density > r.local_density; }
...
public:
	...
	void joint_local_density_computation(const float cutoff) {
...
struct sc_pair {
	...
	bool operator>(const sc_pair& p) const { return cost > p.cost; }
};
